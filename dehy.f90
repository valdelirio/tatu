...
contains
	subroutine dehy_xyz(filAnd,Tx,Ty,Iw,dsy,h0,n,esp,condut,neta,zeta,cx,cy,z,Ex_p,Ey_p,Ez_p,Hx_p,Hy_p,Hz_p)
	implicit none
	...
	if ( cx == Tx .and. Tx /= 0.d0 ) then
        x = dsign( 1.d0, Tx )
    elseif ( cx == Tx .and. Tx == 0.d0 ) then
    	x = 1.d0
	else
        x = cx - Tx
	end if
	if ( cy == Ty .and. Ty /= 0.d0 ) then
		y = dsign(1.d0,Ty)
! 	else if ( cy == Ty .and. Ty == 0.d0 ) then
! 		y = 1.d0
	else
		y = cy - Ty
	end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	TE_dw = - Iw * dsy / (2.d0 * AdmInt(kr,camadT))
	do j=camadT+1,cam	
		if (j == (camadT + 1) .and. j == n) then
...
! 	
! 	if (cam == camadT) then
! 	TEdw = - Iw * dsy / (2.d0 * AdmInt(kr,camadT))
! 	elseif (cam == (camadT + 1) .and. cam == n) then
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	TE_up = - Iw * dsy / (2.d0 * AdmInt(kr,camadT))
	do j=camadT-1,cam,-1
		if (j == (camadT - 1) .and. j == 0) then
...
! 	
! 	if (cam == camadT) then
! 	TEup = - Iw * dsy / (2.d0 * AdmInt(kr,camadT))
! 	elseif (cam == (camadT - 1) .and. cam == 0) then
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEx0_J1 = 2.d0*x*y/r**3 * (Ktmdz_0(kr) + Kte_0(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEx0_J0 = x*y/r**2 * (Ktmdz_0(kr) + Kte_0(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExk_J1 = 2.d0*x*y/r**3 * (Ktmdz_k(kr,camad) + Kte_k(kr,camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExk_J0 = x*y/r**2 * (Ktmdz_k(kr,camad) + Kte_k(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExl_up_J1 = 2.d0*x*y/r**3 * (Ktmdz_l_up(kr) + Kte_l_up(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExl_up_J0 = x*y/r**2 * (Ktmdz_l_up(kr) + Kte_l_up(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExl_dw_J1 = 2.d0*x*y/r**3 * (-Ktmdz_l_dw(kr) + Kte_l_dw(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExl_dw_J0 = x*y/r**2 * (-Ktmdz_l_dw(kr) + Kte_l_dw(kr)) * kr 
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExj_J1 = 2.d0*x*y/r**3 * (-Ktmdz_j(kr,camad) + Kte_j(kr,camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExj_J0 = x*y/r**2 * (-Ktmdz_j(kr,camad) + Kte_j(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExn_J1 = 2.d0*x*y/r**3 * (-Ktmdz_n(kr) + Kte_n(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KExn_J0 = x*y/r**2 * (-Ktmdz_n(kr) + Kte_n(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEy0_J1 = (2.d0*y**2/r**3 - 1/r) * Ktmdz_0(kr) - (2.d0*x**2/r**3 - 1/r) * Kte_0(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEy0_J0 = (y**2/r**2 * Ktmdz_0(kr) - x**2/r**2 * Kte_0(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyk_J1 = (2.d0*y**2/r**3 - 1/r) * Ktmdz_k(kr,camad) - (2.d0*x**2/r**3 - 1/r) * Kte_k(kr,camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyk_J0 = (y**2/r**2 * Ktmdz_k(kr,camad) - x**2/r**2 * Kte_k(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyl_up_J1 = (2.d0*y**2/r**3 - 1/r) * Ktmdz_l_up(kr) - (2.d0*x**2/r**3 - 1/r) * Kte_l_up(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyl_up_J0 = (y**2/r**2 * Ktmdz_l_up(kr) - x**2/r**2 * Kte_l_up(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyl_dw_J1 = (1/r - 2.d0*y**2/r**3) * Ktmdz_l_dw(kr) + (1/r - 2.d0*x**2/r**3) * Kte_l_dw(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyl_dw_J0 = (y**2/r**2 * Ktmdz_l_dw(kr) + x**2/r**2 * Kte_l_dw(kr)) * kr 
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyj_J1 = (1/r - 2.d0*y**2/r**3) * Ktmdz_j(kr,camad) + (1/r - 2.d0*x**2/r**3) * Kte_j(kr,camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyj_J0 = (y**2/r**2 * Ktmdz_j(kr,camad) + x**2/r**2 * Kte_j(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyn_J1 = (1/r - 2.d0*y**2/r**3) * Ktmdz_n(kr) + (1/r - 2.d0*x**2/r**3) * Kte_n(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEyn_J0 = (y**2/r**2 * Ktmdz_n(kr) + x**2/r**2 * Kte_n(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEz0_J1 = y * Ktm_0(kr) * kr * kr / (r * neta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEzk_J1 = y * Ktm_k(kr,camad) * kr * kr / (r * condut(camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	if (camad /= 0) then
	KEzl_up_J1 = y * Ktm_l_up(kr) * kr * kr / (r * condut(camad))
	else
	KEzl_up_J1 = y * Ktm_l_up(kr) * kr * kr / (r * neta)
	end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	if (camad /= 0) then
	KEzl_dw_J1 = y * Ktm_l_dw(kr) * kr * kr / (r * condut(camad))
	else
	KEzl_dw_J1 = y * Ktm_l_dw(kr) * kr * kr / (r * neta)
	end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEzj_J1 = y * Ktm_j(kr,camad) * kr * kr / (r * condut(camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEzn_J1 = y * Ktm_n(kr) * kr * kr / (r * condut(n))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHx0_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_0(kr) - (2.d0*x**2/r**3 -1/r) * Ktedz_0(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHx0_J0 = (y**2/r**2 * Ktm_0(kr) - x**2/r**2 * Ktedz_0(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxk_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_k(kr,camad) - (2.d0*x**2/r**3 -1/r) * Ktedz_k(kr,camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxk_J0 = (y**2/r**2 * Ktm_k(kr,camad) - x**2/r**2 * Ktedz_k(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxl_up_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_l_up(kr) - (2.d0*x**2/r**3 -1/r) * Ktedz_l_up(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxl_up_J0 = (y**2/r**2 * Ktm_l_up(kr) - x**2/r**2 * Ktedz_l_up(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxl_dw_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_l_dw(kr) + (2.d0*x**2/r**3 -1/r) * Ktedz_l_dw(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxl_dw_J0 = (y**2/r**2 * Ktm_l_dw(kr) + x**2/r**2 * Ktedz_l_dw(kr)) * kr 
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxj_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_j(kr,camad) + (2.d0*x**2/r**3 -1/r) * Ktedz_j(kr,camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxj_J0 = (y**2/r**2 * Ktm_j(kr,camad) + x**2/r**2 * Ktedz_j(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxn_J1 = (2.d0*y**2/r**3 -1/r) * Ktm_n(kr) + (2.d0*x**2/r**3 -1/r) * Ktedz_n(kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHxn_J0 = (y**2/r**2 * Ktm_n(kr) + x**2/r**2 * Ktedz_n(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHy0_J1 = 2.d0*x*y/r**3 * (Ktm_0(kr) + Ktedz_0(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHy0_J0 = x*y/r**2 * (Ktm_0(kr) + Ktedz_0(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyk_J1 = 2.d0*x*y/r**3 * (Ktm_k(kr,camad) + Ktedz_k(kr,camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyk_J0 = x*y/r**2 * (Ktm_k(kr,camad) + Ktedz_k(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyl_up_J1 = 2.d0*x*y/r**3 * (Ktm_l_up(kr) + Ktedz_l_up(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyl_up_J0 = x*y/r**2 * (Ktm_l_up(kr) + Ktedz_l_up(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyl_dw_J1 = 2.d0*x*y/r**3 * (Ktm_l_dw(kr) - Ktedz_l_dw(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyl_dw_J0 = x*y/r**2 * (Ktm_l_dw(kr) - Ktedz_l_dw(kr)) * kr 
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyj_J1 = 2.d0*x*y/r**3 * (Ktm_j(kr,camad) - Ktedz_j(kr,camad))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyj_J0 = x*y/r**2 * (Ktm_j(kr,camad) - Ktedz_j(kr,camad)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyn_J1 = 2.d0*x*y/r**3 * (Ktm_n(kr) - Ktedz_n(kr))
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHyn_J0 = x*y/r**2 * (Ktm_n(kr) - Ktedz_n(kr)) * kr
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHz0_J1 = x * Kte_0(kr) * kr * kr / (r * zeta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHzk_J1 = x * Kte_k(kr,camad) * kr * kr / (r * zeta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHzl_up_J1 = x * Kte_l_up(kr) * kr * kr / (r * zeta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHzl_dw_J1 = x * Kte_l_dw(kr) * kr * kr / (r * zeta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHzj_J1 = x * Kte_j(kr,camad) * kr * kr / (r * zeta)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KHzn_J1 = x * Kte_n(kr) * kr * kr / (r * zeta)

	return
	end function KHzn_J1
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	end subroutine dehy_xyz
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	subroutine dehy_xkyz(Tx,ky,Iw,dsy,h0,n,esp,condut,neta,zeta,cx,z,Ex_ky,Ey_ky,Ez_ky,Hx_ky,Hy_ky,Hz_ky)
	implicit none
	integer(4),intent(in)::n
	real(8),intent(in)::Tx,ky,Iw,dsy,h0,esp(:),condut(1:n),cx,z    !,neta
	complex*16,intent(in)::zeta,neta
	...
	if (camad == 0 .and. camadT /= 0) then
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHz0(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
	...
	elseif (camad < camadT) then !camada k
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHzk(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
	...
	elseif (camad == camadT .and. z <= h0) then	!na mesma camada do transmissor mas acima dele
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHzl_up(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
	...
	elseif (camad == camadT .and. z > h0) then	!na mesma camada do transmissor mas abaixo dele
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHzl_dw(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
	...
	elseif (camad > camadT .and. camad /= n) then !camada j
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHzj(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
	...
	elseif (camad == n .and. camadT /= n) then	!camada n
		...
		kerHz = (0.d0,0.d0)
			do i=1,npts
				...
				kerHz = kerHz + KHzn(kx,ky)*w_sen(i)
			end do
		kerEy = (0.d0,0.d0)
		kerEz = (0.d0,0.d0)
		kerHx = (0.d0,0.d0)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	KEx0 = - kx * ky * (Ktmdz_0(kr) + Kte_0(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KExk = - kx * ky * (Ktmdz_k(kr,camad) + Kte_k(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KExl_up = - kx * ky * (Ktmdz_l_up(kr) + Kte_l_up(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KExl_dw = kx * ky * (Ktmdz_l_dw(kr) - Kte_l_dw(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KExj = kx * ky * (Ktmdz_j(kr,camad) - Kte_j(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KExn = kx * ky * (Ktmdz_n(kr) - Kte_n(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEy0 = (- ky * ky * Ktmdz_0(kr) + kx * kx * Kte_0(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEyk = (- ky * ky * Ktmdz_k(kr,camad) + kx * kx * Kte_k(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEyl_up = (- ky * ky * Ktmdz_l_up(kr) + kx * kx * Kte_l_up(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEyl_dw = (ky * ky * Ktmdz_l_dw(kr) + kx * kx * Kte_l_dw(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEyj = (ky * ky * Ktmdz_j(kr,camad) + kx * kx * Kte_j(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEyn = (ky * ky * Ktmdz_n(kr) + kx * kx * Kte_n(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEz0 = dcmplx(0.d0,ky) * Ktm_0(kr) / neta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEzk = dcmplx(0.d0,ky) * Ktm_k(kr,camad) / condut(camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	if (camad /= 0) then
	KEzl_up = dcmplx(0.d0,ky) * Ktm_l_up(kr) / condut(camad)
	else
	KEzl_up = dcmplx(0.d0,ky) * Ktm_l_up(kr) / neta
	end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	if (camad /= 0) then
	KEzl_dw = dcmplx(0.d0,ky) * Ktm_l_dw(kr) / condut(camad)
	else
	KEzl_dw = dcmplx(0.d0,ky) * Ktm_l_dw(kr) / neta
	end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEzj = dcmplx(0.d0,ky) * Ktm_j(kr,camad) / condut(camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KEzn = dcmplx(0.d0,ky) * Ktm_n(kr) / condut(n)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHx0 = (- ky * ky * Ktm_0(kr) + kx * kx * Ktedz_0(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHxk = (- ky * ky * Ktm_k(kr,camad) + kx * kx * Ktedz_k(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHxl_up = (- ky * ky * Ktm_l_up(kr) + kx * kx * Ktedz_l_up(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHxl_dw = (- ky * ky * Ktm_l_dw(kr) - kx * kx * Ktedz_l_dw(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHxj = (- ky * ky * Ktm_j(kr,camad) - kx * kx * Ktedz_j(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHxn = (- ky * ky * Ktm_n(kr) - kx * kx * Ktedz_n(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHy0 = kx * ky * (Ktm_0(kr) + Ktedz_0(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHyk = kx * ky * (Ktm_k(kr,camad) + Ktedz_k(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHyl_up = kx * ky * (Ktm_l_up(kr) + Ktedz_l_up(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHyl_dw = kx * ky * (Ktm_l_dw(kr) - Ktedz_l_dw(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHyj = kx * ky * (Ktm_j(kr,camad) - Ktedz_j(kr,camad)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHyn = kx * ky * (Ktm_n(kr) - Ktedz_n(kr)) / (kr*kr)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHz0 = dcmplx(0.d0,-kx) * Kte_0(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHzk = dcmplx(0.d0,-kx) * Kte_k(kr,camad) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHzl_up = dcmplx(0.d0,-kx) * Kte_l_up(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHzl_dw = dcmplx(0.d0,-kx) * Kte_l_dw(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHzj = dcmplx(0.d0,-kx) * Kte_j(kr,camad) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	kr = dsqrt(kx*kx+ky*ky)
	KHzn = dcmplx(0.d0,-kx) * Kte_n(kr) / zeta

	return
	end function KHzn
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	end subroutine dehy_xkyz
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	subroutine dehy_xyz_loops(Tx,Ty,Iw,dsy,h0,n,esp,condut,neta,zeta,cx,cy,z,Ex_p,Ey_p,Ez_p,Hx_p,Hy_p,Hz_p)
	implicit none
	integer(4),intent(in)::n
	real(8),intent(in)::Tx,Ty,Iw,dsy,h0,esp(:),condut(1:n),cx,cy,z !,neta
	complex*16,intent(in)::zeta,neta
	...
	real(8)::x,y,r,kr,k_r
	real(8),dimension(:),allocatable::h,krJ0,krJ1,w_J0,w_J1,prof
	integer(4) :: ehsingx, ehsingy, ehsingxy
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
	subroutine dehy_xkyz_loops(Tx,ky,Iw,dsy,h0,n,esp,condut,neta,zeta,cx,z,Ex_ky,Ey_ky,Ez_ky,Hx_ky,Hy_ky,Hz_ky)
	implicit none
	integer(4),intent(in)::n
	real(8),intent(in)::Tx,ky,Iw,dsy,h0,esp(:),condut(1:n),cx,z    !,neta
	complex*16,intent(in)::zeta,neta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	allocate(TMdwSen(npts,camadT:camad),TMdwCos(nptc,camadT:camad),TEdwSen(npts,camadT:camad),TEdwCos(nptc,camadT:camad))
		do j=camadT,camad
			if (j == camadT) then
			TMdwSen(:,j)= - Iw * dsy / 2.d0
			TMdwCos(:,j)= - Iw * dsy / 2.d0
			TEdwSen(:,j)= - Iw * dsy / (2.d0 * AdmIntSen(:,camadT))
			TEdwCos(:,j)= - Iw * dsy / (2.d0 * AdmIntCos(:,camadT))
			elseif (j == (camadT + 1) .and. j == n) then
	...
	allocate(TMupSen(npts,camad:camadT),TMupCos(nptc,camad:camadT),TEupSen(npts,camad:camadT),TEupCos(nptc,camad:camadT))
		do j=camadT,camad,-1
			if (j == camadT) then
			TMupSen(:,j)= Iw * dsy / 2.d0
			TMupCos(:,j)= Iw * dsy / 2.d0
			TEupSen(:,j)= - Iw * dsy / (2.d0 * AdmIntSen(:,camadT))
			TEupCos(:,j)= - Iw * dsy / (2.d0 * AdmIntCos(:,camadT))
			elseif (j == (camadT - 1) .and. j == 0) then
	...
	allocate(TMdwSen(npts,camadT:camad),TMdwCos(nptc,camadT:camad),TEdwSen(npts,camadT:camad),TEdwCos(nptc,camadT:camad))
	allocate(TMupSen(npts,camad:camadT),TMupCos(nptc,camad:camadT),TEupSen(npts,camad:camadT),TEupCos(nptc,camad:camadT))
			TMdwSen(:,camad)= - Iw * dsy / 2.d0
			TMdwCos(:,camad)= - Iw * dsy / 2.d0
			TEdwSen(:,camad)= - Iw * dsy / (2.d0 * AdmIntSen(:,camadT))
			TEdwCos(:,camad)= - Iw * dsy / (2.d0 * AdmIntCos(:,camadT))
			TMupSen(:,camad)= Iw * dsy / 2.d0
			TMupCos(:,camad)= Iw * dsy / 2.d0
			TEupSen(:,camad)= - Iw * dsy / (2.d0 * AdmIntSen(:,camadT))
			TEupCos(:,camad)= - Iw * dsy / (2.d0 * AdmIntCos(:,camadT))
	end if
	...
	if (camad == 0 .and. camadT /= 0) then
		...
		kernelEz = dcmplx(0.d0,ky) * Ktm_Cos * w_cos
		...
		kernelHz = (dcmplx(0.d0,-kxsen) / zeta * Kte_Sen) * w_Sen
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	end subroutine dehy_xkyz_loops
end module DEHy