...
contains
  ...
  subroutine dehy_xyz_loops(Tx,Ty,h0,n,esp,condut,neta,zeta,cx,cy,z,Ex_p,Ey_p,Ez_p,Hx_p,Hy_p,Hz_p)
  implicit none
  integer,intent(in)::n
  ...
  ! real(dp), parameter :: dsy = 1.d0 !momento
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  subroutine dehy_xkyz_loops(Tx,ky,h0,n,esp,condut,neta,zeta,cx,z,Ex_ky,Ey_ky,Ez_ky,Hx_ky,Hy_ky,Hz_ky)
  implicit none
  integer,intent(in)::n
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  subroutine dehy_xkyz(Tx,ky,h0,n,esp,condut,neta,zeta,cx,z,Ex_ky,Ey_ky,Ez_ky,Hx_ky,Hy_ky,Hz_ky)
  implicit none
  integer,intent(in)::n
  real(dp),intent(in)::Tx,ky,h0,esp(:),condut(1:n),cx,z    !,neta
  complex(dp),intent(in)::zeta,neta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KEz0 = cmplx(0.d0,ky,kind=dp) * Ktm_0(kr) / neta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KEzk = cmplx(0.d0,ky,kind=dp) * Ktm_k(kr,camad) / condut(camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  if (camad /= 0) then
  KEzl_up = cmplx(0.d0,ky,kind=dp) * Ktm_l_up(kr) / condut(camad)
  else
  KEzl_up = cmplx(0.d0,ky,kind=dp) * Ktm_l_up(kr) / neta
  end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  if (camad /= 0) then
  KEzl_dw = cmplx(0.d0,ky,kind=dp) * Ktm_l_dw(kr) / condut(camad)
  else
  KEzl_dw = cmplx(0.d0,ky,kind=dp) * Ktm_l_dw(kr) / neta
  end if
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KEzj = cmplx(0.d0,ky,kind=dp) * Ktm_j(kr,camad) / condut(camad)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KEzn = cmplx(0.d0,ky,kind=dp) * Ktm_n(kr) / condut(n)
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHz0 = cmplx(0.d0,-kx,kind=dp) * Kte_0(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHzk = cmplx(0.d0,-kx,kind=dp) * Kte_k(kr,camad) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHzl_up = cmplx(0.d0,-kx,kind=dp) * Kte_l_up(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHzl_dw = cmplx(0.d0,-kx,kind=dp) * Kte_l_dw(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHzj = cmplx(0.d0,-kx,kind=dp) * Kte_j(kr,camad) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  kr = dsqrt(kx*kx+ky*ky)
  KHzn = cmplx(0.d0,-kx,kind=dp) * Kte_n(kr) / zeta
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
  ...
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  !   subroutine dehy_xyz(filAnd,Tx,Ty,Iw,h0,n,esp,condut,neta,zeta,cx,cy,z,Ex_p,Ey_p,Ez_p,Hx_p,Hy_p,Hz_p)
  !   implicit none
  !   integer,intent(in)::filAnd,n
  !   real(dp),intent(in)::Tx,Ty,Iw,h0,esp(:),condut(1:n),cx,cy,z !,neta
  !   complex(dp),intent(in)::zeta,neta
  !   complex(dp),intent(out)::Ex_p,Ey_p,Ez_p,Hx_p,Hy_p,Hz_p
  !
  ! real(dp),parameter::pi=3.141592653589793238462643383279502884197d0
