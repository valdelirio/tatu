...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	implicit none
	integer(4), intent(in) :: tipofiltro, criador
	integer(4), intent(out) :: idtfcd_f, np_f

	integer(4) :: idtfcdf, n_J0, n_J1, n_sen, n_cos
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	real(8), dimension(:), allocatable :: kk_abs_601, kk_pes_sen601, kk_pes_cos601, kk_abs_101, kk_pes_sen101, kk_pes_cos101
	real(8), dimension(:), allocatable :: kk_abs_201, kk_pes_sen201, kk_pes_cos201
	
    real(8), dimension(:), allocatable :: fl_abs_J070, fl_pes_J070, fl_abs_J170, fl_pes_J170	
...
!-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
	...
	implicit none
	real(8), intent(in) :: r
	integer(4), intent(in) :: uso, tipofiltro, criador, idtfcd_f, np_f
	real(8), intent(out) :: Kr( np_f ), w( np_f )
	real(8) :: abs_f( np_f )
