...
subroutine hmdy_optimized(Tx, Ty, h0, n, camadR, camadT, npt, krJ0J1, wJ0, wJ1, h, prof, eta, zeta, eta0, cx, cy, z, &
                          u, uh, AdmInt, ImpInt, RTEdw, RTEup, RTMdw, RTMup, FEdw, FEup, AMdw, AMup, &
                          Ex_p, Ey_p, Ez_p, Hx_p, Hy_p, Hz_p)
  ! Obtido como rotação (de 90° no sentido anti-horário (de x para y)) do hmdx em torno do eixo z. As mudanças são as seguintes:
  ...
  if (camadR > camadT) then
    ...
    do j=camadT,camadR
      if (j == camadT) then
        TMdw(:,j)= zeta * my / (2.0 * ImpInt(:,camadT))
        TEdw(:,j)= -zeta * my / 2.0
  ...
  elseif (camadR < camadT) then
    ...
    do j=camadT,camadR,-1
      if (j == camadT) then
        TMup(:,j) = zeta * my / (2.0 * ImpInt(:,camadT))
        TEup(:,j) = zeta * my / 2.0
    ...
    allocate(TMdw(npt,camadT:camadR), TEdw(npt,camadT:camadR))
    allocate(TMup(npt,camadR:camadT), TEup(npt,camadR:camadT))
    TMdw(:,camadR) = zeta * my / (2.0 * ImpInt(:,camadT))
    TEdw(:,camadR) =-zeta * my / 2.0
  ...
  if (camadR == 0 .and. camadT /= 0) then
    ...
    kernelEyJ1 = -xy_r2 * sum(Ktmdz_J1 - Kte_J1) / r
    kernelEyJ0 = -xy_r2 * sum((Ktmdz_J0 - Kte_J0) * kr) / 2.0
    Ey_p = (kernelEyJ1 - kernelEyJ0) / pi / r

    kernelExJ1 = (twox2_r2m1 * sum(Ktmdz_J1) + twoy2_r2m1 * sum(Kte_J1)) / r
    kernelExJ0 = sum((x2_r2 * Ktmdz_J0 + y2_r2 * Kte_J0) * kr)
    Ex_p = (kernelExJ0 - kernelExJ1) / twopir
    ...
    kernelHxJ1 = -xy_r2 * sum(Ktm_J1 - Ktedz_J1) / r
    kernelHxJ0 = -xy_r2 * sum((Ktm_J0 - Ktedz_J0) * kr) / 2.0
    Hx_p = (kernelHxJ1 - kernelHxJ0) / pi / r
  ...
  elseif (camadR == camadT .and. z > h0) then  !na mesma camada do transmissor mas abaixo dele
    ...
    kernelEyJ1 = -xy_r2 * sum(Ktmdz_J1 + Kte_J1) / r
    kernelEyJ0 = -xy_r2 * sum((Ktmdz_J0 + Kte_J0) * kr) / 2.0
    Ey_p = (kernelEyJ0 - kernelEyJ1) / pi / r

    kernelExJ1 = (twoy2_r2m1 * sum(Kte_J1) - twox2_r2m1 * sum(Ktmdz_J1)) / r
    kernelExJ0 = sum((x2_r2 * Ktmdz_J0 - y2_r2 * Kte_J0) * kr)
    ...
    kernelHyJ1 = (twoy2_r2m1 * sum(Ktedz_J1) - twox2_r2m1 * sum(Ktm_J1)) / r
    kernelHyJ0 = sum((x2_r2 * Ktm_J0 - y2_r2 * Ktedz_J0) * kr)
    Hy_p = -(kernelHyJ1 + kernelHyJ0) / twopir
...
end subroutine hmdy_optimized !obtido como rotação (de 90° no sentido anti-horário (de x para y)) do hedx em torno do eixo z
