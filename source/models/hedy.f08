...
subroutine hedy_optimized(Tx, Ty, h0, n, camadR, camadT, npt, krJ0J1, wJ0, wJ1, h, prof, eta, zeta, eta0, cx, cy, z, &
                          u, uh, AdmInt, ImpInt, RTEdw, RTEup, RTMdw, RTMup, FEdw, FEup, AMdw, AMup, &
                          Ex_p, Ey_p, Ez_p, Hx_p, Hy_p, Hz_p)
  ! Obtido como rotação (de 90° no sentido anti-horário (de x para y)) do hedx em torno do eixo z. As mudanças são as seguintes:
  ! Nas coordenadas: hedx -----> hedy
  !                    x  ----->  y
  !                    y  -----> -x
  ! Nas componentes: hedx -----> hedy
  !                   Ex  ----->  Ey
  !                  -Ey  ----->  Ex
  !                   Hx  ----->  Hy
  !                  -Hy  ----->  Hx
  ...
  if (camadR > camadT) then
    ...
    do j=camadT,camadR
      if (j == camadT) then
        TMdw(:,j)= - Iw * dsy / 2.0
        TEdw(:,j)= - Iw * dsy / (2.0 * AdmInt(:,camadT))
  ...
  elseif (camadR < camadT) then
    ...
    do j=camadT,camadR,-1
      if (j == camadT) then
        TMup(:,j)= Iw * dsy / 2.0
        TEup(:,j)= - Iw * dsy / (2.0 * AdmInt(:,camadT))
    ...
    allocate(TMdw(npt,camadT:camadR), TEdw(npt,camadT:camadR))
    allocate(TMup(npt,camadR:camadT), TEup(npt,camadR:camadT))
    TMdw(:,camadR)= - Iw * dsy / 2.0
    TEdw(:,camadR)= - Iw * dsy / (2.0 * AdmInt(:,camadT))
    TMup(:,camadR)= Iw * dsy / 2.0
    TEup(:,camadR)= - Iw * dsy / (2.0 * AdmInt(:,camadT))
  end if
  ...
  if (camadR == 0 .and. camadT /= 0) then
    ...
    kernelEyJ1 = (twoy2_r2m1 * sum(Ktmdz_J1) - twox2_r2m1 * sum(Kte_J1)) / r
    kernelEyJ0 = sum((y2_r2 * Ktmdz_J0 - x2_r2 * Kte_J0) * kr)
    Ey_p = (kernelEyJ1 - kernelEyJ0) / twopir

    kernelExJ1 = -2.0 * xy_r2 * sum(Ktmdz_J1 + Kte_J1) / r
    kernelExJ0 = -xy_r2 * sum((Ktmdz_J0 + Kte_J0) * kr)
    Ex_p = (-kernelExJ1 + kernelExJ0) / twopir

    kernelEzJ1 = y * sum(Ktm_J1 * kr * kr) / r / eta0
    Ez_p = - kernelEzJ1 / twopir

    kernelHyJ1 = -2.0 * xy_r2 * sum(Ktm_J1 + Ktedz_J1) / r
    kernelHyJ0 = -xy_r2 * sum((Ktm_J0 + Ktedz_J0) * kr)
    Hy_p = (kernelHyJ1 - kernelHyJ0) / twopir

    kernelHxJ1 = (-twoy2_r2m1 * sum(Ktm_J1) + twox2_r2m1 * sum(Ktedz_J1)) / r
    kernelHxJ0 = sum((y2_r2 * Ktm_J0 - x2_r2 * Ktedz_J0) * kr)
    Hx_p = -(kernelHxJ1 + kernelHxJ0) / twopir

    kernelHzJ1 = x * sum(Kte_J1 * kr * kr) / r / zeta
  ...
  elseif (camadR < camadT) then !camada k
    ...
    kernelEzJ1 = y * sum(Ktm_J1 * kr * kr) / r / eta(camadR)
    Ez_p = -kernelEzJ1 / twopir
  ...
  elseif (camadR > camadT .and. camadR /= n) then !camada j
    ...
    kernelEzJ1 = y * sum(Ktm_J1 * kr * kr) / r / eta(n)
    Ez_p = -kernelEzJ1 / twopir
...
end subroutine hedy_optimized !obtido como rotação (de 90° no sentido anti-horário (de x para y)) do hedx em torno do eixo z
